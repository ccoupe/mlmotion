#
#  This is a 'light' version of mqtt-motion-video. Only usable with
#  net connected mlshape or deepstack server for object detection
#  1. No Local AI - always uses a server.
#  2. No rpyc. 
#  3. No Lux calc. 
#
import os
...
import websocket as wscl # websocket-client
import base64
import requests
...
# TODO: upper limit on waiting? 
'''
  In check_presense() determine if we are being called in a 'bursty'
  manner. Example: Stoic_cam gets lots of motion events but doesn't have
  a camera view that gets a True from shape_check() unless the subject rotates
  180 degrees. 2 attempts per sec, for 3 sets is that bursty?
  
  Then what? increase delay? It's only stoic.local and there is plenty of cpu
  to spare - so why bother? 
'''
...
def next_state(nevent):
  ...
  if nevent == Event.motion:
    if cur_state == State.disabled:
      ...
      if settings.two_step:
        ...
        if tf:
          ...
          # TODO: upper limit on waiting? How? 
      ...
      if motion_cnt <= 0:
        ...
        if settings.two_step:
          ...
          if not tf:
            ...
            # still someone there, keep holding for another 'hold' ticks.
            motion_cnt = settings.active_hold      
            next_st = State.motion_hold
      else:
        next_st = State.motion_hold
    ...
    # Happens when external process (like hubitat - turns off the switch
...
# Call CNN_Shapes server
def ws_check_presence(ip, port, frame):
  # this one uses a websocket to the server, gets a json return
  # Returns json['value'] , a boolean.
  global settings, applog
  ws = wscl.WebSocket()

  try:
    uri = f'ws://{ip}:{port}/Cnn_Shapes'
    ...
    applog.warning(f'Trying backup ip for this {ip}:{port}')
    raise MLServerError    
  # Shouldn't get here
  return False
  
# call Deepstack for object detect
def ds_check_presence(ip, port, frame):
  global settings, applog
  response = []
  _, jpgbfr = cv2.imencode('.jpg',frame)
  try: 
    response = requests.post(f"http://{ip}:{port}/v1/vision/detection",
        files={"image":jpgbfr}).json()
  except (requests.exceptions.ConnectionError):
    applog.warning(f'requests ConnectionError from {ip}:{port}')
    raise MLServerError

  #applog.info(f"back from {ip}:{port} => {response}")
  if response['success'] == True:
    for object in response["predictions"]:
      if object['label'] == 'person' and object['confidence'] >= 0.80:
        rect = [ object['x_min'], object['y_min'], object['x_max'], object['y_max']]
        return True
  return False
...
def check_presence():
  ...
  result = False
  st = datetime.now()
  for srvh in settings.ml_servers:
    try:
      host = srvh['host']
      port = srvh['port']
      typ = srvh['type']
      if typ == 'deepstack':
        result = ds_check_presence(host,port,frame1)
        break
      elif typ == 'websocket':
        result = ws_check_presence(host,port,frame1)
        break;
      else:
        applog.warning(f"unknown server {srvh}")
...
#
# imstream is adrian's multithreaded capture in imutils
#
...
def main(args=None):
  ...
  settings = Settings(args["conf"], 
                      ...
                      next_state)
  # logging setup 
  # get a name for the log. 
  logname = settings.homie_device 
  ...
  if args['syslog']:
    ...
    logging.basicConfig(level=logging.DEBUG,datefmt="%H:%M:%S",format='%(asctime)s %(message)-40s')
    f = LuxLogFilter()
    applog.addFilter(f)
  ...
  elif settings.camera_type == 'wyze_native':
    auth_info = wyzecam.login(settings.wyze_email, settings.wyze_password)
    account = wyzecam.get_user_info(auth_info)
    video_dev = wyzecam.WyzeIOTC().connect_and_auth(account, camera)
    read_cam = wyze_read
    read_local_resize = wyze_read_local_resize
    remote_cam = wyze_remote_cam
    camera_spin = wyze_camera_spin
    camera_capture_to_file = wyze_camera_capture_to_file
  else:
