...
class Settings:
  ...
  def load_settings(self, fn):
    ...
    self.wyze_email = conf.get('wyze_email', 'badDude@example.com')
    self.wyze_password = conf.get('wyze_password', 'yourpassword')
    #self.ml_backup_ip = conf.get('ml_backup_ip', '192.168.1.2')
    self.ml_servers = conf.get('ml_servers', [])

    if self.use_ml == 'websocket':
      self.ml_port = 4439
    elif self.use_ml == 'remote':
      self.ml_port = 4466
    elif self.use_ml == 'deepstack':
      self.ml_port = 4425
    else:
      print('FAILURE IN Settings, use_ml')
  ...
  def settings_serialize(self):
    ...
    st['wyze_email'] = self.wyze_email
    st['wyze_password'] = self.wyze_password
    str = json.dumps(st)
    return str
